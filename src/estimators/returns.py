...
class ReturnEstimator(ABC):
    ...
    @abstractmethod
    def estimate(self, price_window: np.ndarray,t: int, lookback: int, cov_matrix: np.ndarray) -> np.ndarray:
        pass
...
class EquilibriumReturnEstimator(ReturnEstimator):
    def __init__(self, market_index_prices: np.ndarray, risk_free_rates: np.ndarray, market_caps: np.ndarray):
        """

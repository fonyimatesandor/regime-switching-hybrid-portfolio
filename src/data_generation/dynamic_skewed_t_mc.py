...
class DynamicSkewedTSimulator(BaseMonteCarloSimulator):
    ...
    def fit(self, asset_prices: pd.DataFrame, factors: pd.DataFrame) -> None:
        ...
        joint_log_returns = (
            np.concatenate((asset_log_returns, factor_log_returns), axis=1) * 100.0
        )
        ...
        for i in range(d):
            ...
            res = model.fit(
                disp="off", options={"maxiter": self.maxiter}, show_warning=False
            )
    ...
    def simulate(
        ...
        for sim in range(num_simulations):
            ...
            for step in range(num_steps - 1):
                ...
                Q_t = (
                    (1 - self.dcc_a - self.dcc_b) * self.Qbar
                    + self.dcc_a * np.outer(Y_prev, Y_prev)
                    + self.dcc_b * Q_t
                )

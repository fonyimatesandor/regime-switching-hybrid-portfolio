...
class StaticNormalSimulator(BaseMonteCarloSimulator):
    """Monte Carlo simulator that assumes joint normal distribution of asset and factor log returns, with static parameters estimated from historical data."""
    ...
    def simulate(
        ...
        joint_sim_log_returns = np.random.multivariate_normal(
            self.means, self.cov_matrix, size=(num_simulations, num_steps - 1)
        )

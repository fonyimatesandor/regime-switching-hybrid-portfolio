...
class MeanVariancePortfolio(BaseStrategy):
    def __init__(self, 
        ...
        if self.objective != 'max_sharpe':
            ...
            A_rows = [In, ones]
            l_rows = [self.lb, np.array([1.0])]
            u_rows = [self.ub, np.array([1.0])]
            
            for c in self.osqp_static:
                A_rows.append(sp.csc_matrix(c['A']))
                l_rows.append(np.where(np.isfinite(c['l']), c['l'], -osqp.constant('OSQP_INFTY')))
                u_rows.append(np.where(np.isfinite(c['u']), c['u'], osqp.constant('OSQP_INFTY')))
                
            self.A = sp.vstack(A_rows, format='csc')
            self.l = np.concatenate(l_rows)
            self.u = np.concatenate(u_rows)
            
            self.mvo_solver = None  
            
            
        else:
            A_sum = sp.hstack([sp.csc_matrix(np.ones((1, self.num_assets))), sp.csc_matrix([[-1.0]])], format='csc')
            
            In = sp.eye(self.num_assets, format='csc')
            A_lb = sp.hstack([In, sp.csc_matrix(-self.lb).T], format='csc')
            A_ub = sp.hstack([In, sp.csc_matrix(-self.ub).T], format='csc')
            
            A_kappa = sp.hstack([sp.csc_matrix(np.zeros((1, self.num_assets))), sp.csc_matrix([[1.0]])], format='csc')
            
            A_rows = [A_sum, A_lb, A_ub, A_kappa]
            l_rows = [np.array([0.0]), np.zeros(self.num_assets), np.full(self.num_assets, -osqp.constant('OSQP_INFTY')), np.array([1e-8])]
            u_rows = [np.array([0.0]), np.full(self.num_assets, osqp.constant('OSQP_INFTY')), np.zeros(self.num_assets), np.array([osqp.constant('OSQP_INFTY')])]
            ...
            for c in self.osqp_static:
                ...
                if np.any(valid_l):
                    A_rows.append(sp.hstack([Ac[valid_l, :], sp.csc_matrix(-lc[valid_l]).T], format='csc'))
                ...
                if np.any(valid_u):
                    A_rows.append(sp.hstack([Ac[valid_u, :], sp.csc_matrix(-uc[valid_u]).T], format='csc'))
                    l_rows.append(np.full(np.sum(valid_u), -osqp.constant('OSQP_INFTY')))
                    u_rows.append(np.zeros(np.sum(valid_u)))

            self.A_static = sp.vstack(A_rows, format='csc')
            
            self.l = np.concatenate([np.array([1.0]), np.concatenate(l_rows)])
            self.u = np.concatenate([np.array([1.0]), np.concatenate(u_rows)])
            self.mvo_solver = None    
    ...
    def _compute_target_weights(self, period: int) -> np.ndarray:
        ...
        if self.objective == 'max_sharpe':
            ...
            P_upper = sp.bmat([[P_y, None], [None, sp.csc_matrix((1, 1))]], format='csc')
            q = np.zeros(self.num_assets + 1)
            
            A_ret = sp.hstack([sp.csc_matrix(expected_returns), sp.csc_matrix([[0.0]])], format='csc')
            A = sp.vstack([A_ret, self.A_static], format='csc')
            
            if self.mvo_solver is None:
                self.mvo_solver = osqp.OSQP()
                self.mvo_solver.setup(P_upper, q, A, self.l, self.u, verbose=False, polish=True)
            else:
                try:
                    self.mvo_solver.update(Px=P_upper.data, Ax=A.data)
                except ValueError:
                    self.mvo_solver = osqp.OSQP()
                    self.mvo_solver.setup(P_upper, q, A, self.l, self.u, verbose=False, polish=True)
            
            res = self.mvo_solver.solve()
            ...
            if self.mvo_solver is None:
                self.mvo_solver = osqp.OSQP()
                self.mvo_solver.setup(P_upper, q, self.A, self.l, self.u, verbose=False, polish=True)
            else:
                self.mvo_solver.update(Px=P_upper.data, q=q, l=self.l, u=self.u)
                
            result = self.mvo_solver.solve()
    ...
    def compute_efficient_frontier(self, num_points: int = 20):
        ...
        for i, lam in enumerate(lambdas):
            ...
            risks[i] = np.std(temp_model.portfolio_value[1:] / temp_model.portfolio_value[:-1] - 1.0)

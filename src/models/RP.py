...
class RiskParityPortfolio(BaseStrategy):
    ...
    def _setup_cvxpy_problem(self) -> None:
        ...
        self._cov_param = cp.Parameter((N, N), PSD=True)

        sum_x = cp.sum(self._x)
        risk_budget = np.ones(N) / N

        objective = cp.Minimize(
            0.5 * cp.quad_form(self._x, self._cov_param) - risk_budget @ cp.log(self._x)
        )

        constraints = [self._x >= self._lb * sum_x, self._x <= self._ub * sum_x]
        ...
        for c in self._osqp_static:
            ...
            for i in range(len(l)):
                if l[i] > -1e15:
                    constraints.append(A[i] @ self._x >= l[i] * sum_x)
                if u[i] < 1e15:
                    constraints.append(A[i] @ self._x <= u[i] * sum_x)

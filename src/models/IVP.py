...
class InverseVariancePortfolio(BaseStrategy):
    ...
    def _compute_target_weights(self, period: int) -> np.ndarray:
        ...
        returns = (
            np.diff(self.prices[period_start:period], axis=0)
            / self.prices[period_start : period - 1]
        )
        ...
        weights = (
            inv_variances / np.sum(inv_variances)
            if np.sum(inv_variances) > 0
            else np.ones(self.num_assets) / self.num_assets
        )

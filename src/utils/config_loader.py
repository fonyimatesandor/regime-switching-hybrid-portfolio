    ...
    if not path.exists():
        ...
        if not path.exists():
            raise FileNotFoundError(
                "Could not locate config.yaml in current or parent directory."
            )
...
def build_bounds_and_constraints(cfg: dict) -> tuple[list, list, dict]:
    """
    ...
    if "dynamic_constraints" in cfg:
        ...
        if "turnover_limit" in dynamic_cfg:
            dynamic_constraints["max_turnover"] = float(
                dynamic_cfg["turnover_limit"]["max"]
            )

        if "rebalance_threshold" in dynamic_cfg:
            dynamic_constraints["min_diff_to_rebalance"] = float(
                dynamic_cfg["rebalance_threshold"]["min"]
            )

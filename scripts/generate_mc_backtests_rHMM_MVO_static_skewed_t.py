...
for model in rHMM_MVO_models:
    ...
    with open(
        f"data/rhmm_mvo_processed/{model['name']}_{simulator_name}_MC_backtest_results.pkl",
        "wb",
    ) as f:
...
json.dump(
    ...
    open(
        f"data/rhmm_mvo_processed/mc_backtest_summary_rHMM_MVO_{simulator_name}.json",
        "w",
    ),
    indent=4,
)

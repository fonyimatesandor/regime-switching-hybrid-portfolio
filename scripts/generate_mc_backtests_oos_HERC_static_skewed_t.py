...
for model in choosen_HERC_models_oos:
    ...
    with open(
        f"data/oos_processed/{model['name']}_{simulator_name}_MC_backtest_results.pkl",
        "wb",
    ) as f:
...
json.dump(
    {
        "models_tested": [model["name"] for model in choosen_HERC_models_oos],
        "simulators_tested": [simulator_name],
        "config": config,
    },
    open(f"data/oos_processed/mc_backtest_summary_HERC_{simulator_name}.json", "w"),
    indent=4,
)

...
asset_prices = pd.concat([asset_prices_learning, asset_prices_comparison], axis=0)
factors = pd.concat([factors_learning, factors_comparison], axis=0)
...
with open(f"./data/synthetic/{simulator_name}_oos_simulated_data.pkl", "wb") as f:
    pkl.dump((simulator, output), f)

with open(f"./data/synthetic/{simulator_name}_oos_simulator.pkl", "wb") as f:
    pkl.dump(simulator, f)

print(f"{simulator_name} OOS simulator data saved.\n")
